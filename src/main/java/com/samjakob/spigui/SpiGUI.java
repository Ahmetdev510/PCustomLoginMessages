...
public class SpiGUI {
    ...
    private SGPaginationButtonBuilder defaultPaginationButtonBuilder = (type, inventory) -> {
        switch (type) {
            case PREV_BUTTON:
                if (inventory.getCurrentPage() > 0) return new SGButton(new ItemBuilder(Material.getMaterial(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.PrevPageMaterial")))
                        .name(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.PrevPage"))
                        .lore(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.PrevPageLore").replace("{page}", String.valueOf(inventory.getCurrentPage())))
                        .build()
                ).withListener(event -> {
            ...
            case CURRENT_BUTTON:
                return new SGButton(new ItemBuilder(Material.getMaterial(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.CurrentPageMaterial")))
                        .name(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.CurrentPage").replace("{page}", String.valueOf(inventory.getCurrentPage() + 1)).replace("{maxPage}", String.valueOf(inventory.getMaxPage())))
                        .lore(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.CurrentPageLore").replace("{page}",String.valueOf((inventory.getCurrentPage() + 1)))).build()
                ).withListener(event -> event.setCancelled(true));

            case NEXT_BUTTON:
                if (inventory.getCurrentPage() < inventory.getMaxPage() - 1) return new SGButton(new ItemBuilder(Material.getMaterial(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.NextPageMaterial")))
                        .name(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.NextPage"))
                        .lore(PCustomLoginMessages.config.getString(PCustomLoginMessages.instance.getName()+".GUI.NextPageLore").replace("{nextPage}",String.valueOf(inventory.getCurrentPage() + 2))).build()
                ).withListener(event -> {
                    event.setCancelled(true);
